  ...
  const renderDayView = () => {
    ...
    return (
      <div className={styles.dayViewContainer}>
        <h2 className={styles.dayViewDate}>{format(currentDate, 'EEEE, MMMM d, yyyy')}</h2>
    ...
    <div className={styles.monthView}>
      ...
      {daysToDisplay.map((day) => {
        ...
        return (
          <motion.div
            key={format(day, 'yyyy-MM-dd')}
            className={`${styles.monthCard} ${!isCurrentMonth ? styles.otherMonth : ''} ${
              isToday ? styles.today : ''
            }`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            <div className={styles.monthCardNumber}>{format(day, 'd')}</div>
            <div className={styles.monthCardItems}>
              {items.notes.length > 0 && (
    ...
    return (
      <div className={styles.yearView}>
        {months.map((month) => (
          ...
          >
            <h3 className={styles.yearCardTitle}>{format(month.date, 'MMM')}</h3>
            <div className={styles.stat}>
              {month.noteCount > 0 && <span className={styles.statItem}>📝 {month.noteCount}</span>}
              {month.taskCount > 0 && <span className={styles.statItem}>✓ {month.taskCount}</span>}
            </div>
            </motion.div>
          ))}
      </div>
